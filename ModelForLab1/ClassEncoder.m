classdef ClassEncoder < handle
    ...
    properties (SetAccess = private) % Вычисляемые переменные
		% Если есть какие-то переменные нужно посчитать. Тогда нужно инициализировать
		% и посчитать здесь.
			% Для сверточного кода значение скорости кодирования нужно посчитать
				CodeRateCC;
			% Значение скорости кодирования очень важно для определения средней 
			% мощности бита (См. ClassChannel)
    end
    methods
