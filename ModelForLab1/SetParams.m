...
function Encoder = SetParamsEncoder(inEncoder, ParamsNumber, ...
    ...
    % Type of encoder
        if ~isfield(Encoder, 'TypeEncoder')
            Encoder.TypeEncoder = 'Convolution';
        else
            % Проверка корректности введённых значений
        end
    % Type of trellis
        if ~isfield(Encoder, 'Trellis')
    ...
    % Decoding method
        if ~isfield(Encoder, 'DecodingType')
            Encoder.DecodingType = 'hard';
        else
            % Проверка корректности введённых значений
        end
    % Code rate for encoder (used for LDPC)
        if ~isfield(Encoder, 'CodeRate')
